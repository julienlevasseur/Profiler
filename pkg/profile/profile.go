...
// ListFiles return a list of filenames that match the provided extension found
// in the given folder */
...
// SetEnvironment read the profilerFile and set a new environment in the given
// shell (exported one if the config doesn't specify one)
...
// UseNoProfile return a map of all the key:value set found in the local accepted files
func UseNoProfile() {
	envVars := make(map[string]string)
